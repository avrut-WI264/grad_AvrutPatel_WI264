...
public class PropertyManagementApp {
    ...
    private static void authenticate(Connection conn, String requiredRole) {
        ...
        String sql = "SELECT user_id, username, role FROM users WHERE username = ? AND password = ? AND role = ?";

    try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
        pstmt.setString(1, username);
        pstmt.setString(2, password);
        pstmt.setString(3, requiredRole);

        ResultSet rs = pstmt.executeQuery();

        if (rs.next()) {
            // Create a User object to track the session
